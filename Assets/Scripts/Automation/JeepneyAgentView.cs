...
/// </summary>
public class JeepneyAgentView : MonoBehaviour
{
    [Header("References")]
    ...
    public void Init(GridWorldView view, Vector2Int cell, int facing)
    {
        _view = view;
        SnapTo(cell, facing);
    }
    ...
    public IEnumerator PlayAction(AgentActionResult result, float duration)
    {
        switch (result.Action)
        {
            ...
            case "pickUp":
                if (result.PickedUp && _view != null)
                    _view.SetStopOccupied(result.From, false);
    ...
    IEnumerator Bump(int facing, float duration)
    {
        ...
        Vector3 push   = (Vector3)(GridWorldView.FacingScreenDirection(facing) * 0.16f);
    ...
    {
        int order = GridWorldView.SortOrder(cell) + 1;
    ...
    static Quaternion ArrowRotation(int facing)
    {
        Vector2 dir = GridWorldView.FacingScreenDirection(facing);
