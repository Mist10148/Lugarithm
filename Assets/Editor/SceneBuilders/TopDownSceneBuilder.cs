...
public static class TopDownSceneBuilder
{
    ...
    public static void Build()
    {
        ...
        // Optional artifact tracker. It stays hidden until every objective is
        // complete, then switches from X to a check after collection.
        var artifactStatus = UIFactory.CreateRect(statusBackdrop, "ArtifactStatus",
                                                   new Vector2(0f, 1f), new Vector2(0f, 1f));
        UIFactory.Place(artifactStatus, new Vector2(0f, 1f), new Vector2(34f, -192f),
                        new Vector2(292f, 28f));
        var artifactLabel = UIFactory.CreateText(artifactStatus, "Label", "Secret Artifact", 15f,
                                                  UIFactory.TextBright, TextAlignmentOptions.MidlineLeft);
        artifactLabel.rectTransform.offsetMin = Vector2.zero;
        artifactLabel.rectTransform.offsetMax = new Vector2(-42f, 0f);
        var artifactMark = UIFactory.CreateText(artifactStatus, "Mark", "X", 18f,
                                                 UIFactory.Accent, TextAlignmentOptions.Center);
        UIFactory.Place(artifactMark, new Vector2(1f, 0.5f), Vector2.zero, new Vector2(32f, 28f));
        var artifactCheck = UIFactory.CreateText(artifactStatus, "Check", "✓", 18f,
                                                  UIFactory.Accent, TextAlignmentOptions.Center);
        UIFactory.Place(artifactCheck, new Vector2(1f, 0.5f), Vector2.zero, new Vector2(32f, 28f));
