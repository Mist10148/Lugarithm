...
public class AgentSimRideTests
{
    ...
    // -------------------------------------------------------------------------
    // Navigation-sensor reporters in ride mode

    [Test]
    public void NearestStopPosition_PointsAtPickupThenDropoff()
    {
        GridModel grid = StraightGrid();
        var sim = new AgentSim(grid, new FareTable(), startFacing: 1);
        sim.LoadRides(new List<GridRide>
        ...
        Value v = sim.ReadReporter("nearestStopPosition", new List<Value>());
        Assert.AreEqual(ValueKind.Tuple, v.Kind);
        var arr = (Value[])v.Obj;
        Assert.AreEqual(3, arr[0].AsInt());
        ...
        // Now the nearest relevant stop is the drop-off.
        v = sim.ReadReporter("nearestStopPosition", new List<Value>());
        arr = (Value[])v.Obj;
        Assert.AreEqual(5, arr[0].AsInt());
        Assert.AreEqual(1, arr[1].AsInt());
    }

    [Test]
    public void DirectionTo_InRideMode_FollowsShortestPath()
    {
        GridModel grid = StraightGrid();
        var sim = new AgentSim(grid, new FareTable(), startFacing: 1);
        sim.LoadRides(new List<GridRide>
        ...
        // From S(1,1) to pickup (3,1): east.
        Assert.AreEqual(1, sim.ReadReporter("directionTo",
            new List<Value> { Value.Int(3), Value.Int(1) }).AsInt());

        // From S(1,1) to dropoff (5,1): east.
        Assert.AreEqual(1, sim.ReadReporter("directionTo",
            new List<Value> { Value.Int(5), Value.Int(1) }).AsInt());
    }

    [Test]
    public void DistanceTo_MatchesPathLength()
    {
        GridModel grid = StraightGrid();
        var sim = new AgentSim(grid, new FareTable(), startFacing: 1);
        sim.LoadRides(new List<GridRide>
        ...
        // (1,1) -> (3,1): 3 cells inclusive -> distance 3.
        Assert.AreEqual(3, sim.ReadReporter("distanceTo",
            new List<Value> { Value.Int(3), Value.Int(1) }).AsInt());
    }
}
