...
{
    [Test]
    public void EveryPlayableTown_HasFiveStationDefinitions()
    {
        for (int level = 0; level <= 5; level++)
        {
            ...
            Assert.AreEqual(5, defs.Length, $"level {level} station definition count");
            Assert.AreEqual(4, puzzleCount, $"level {level} puzzle station count");
            Assert.AreEqual(1, codingCount, $"level {level} coding station count");
    ...
    [Test]
    public void EveryPlayableTown_MapHasFourPuzzleStationsAndOneCodingStation()
    {
        for (int level = 0; level <= 5; level++)
        {
            ...
            Assert.AreEqual(1, codingStations, $"level {level} C station count");
        }
    }
}
